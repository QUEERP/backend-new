...
async function runTests() {
  try {
    for (const c of TEST_COUNTRIES) {
        ...
        let country = await prisma.country.findFirst({ where: { code: c.code } });
        if (!country) country = await prisma.country.create({ data: { code: c.code, name: 'UAE' } });
        let taxFramework = await prisma.taxFramework.findFirst({ where: { name: c.fw } });
        if (!taxFramework && country) {
            taxFramework = await prisma.taxFramework.create({ data: { name: c.fw, countryId: country.id }});
            const standardType = await prisma.taxType.create({ data: { name: 'VAT_STANDARD', taxFrameworkId: taxFramework.id } });
            await prisma.taxRate.create({ data: { name: 'UAE Standard 5%', rate: 5, effectiveFrom: new Date('2020-01-01'), taxTypeId: standardType.id }});
            const zeroType = await prisma.taxType.create({ data: { name: 'VAT_ZERO', taxFrameworkId: taxFramework.id } });
            await prisma.taxRate.create({ data: { name: 'UAE Zero 0%', rate: 0, effectiveFrom: new Date('2020-01-01'), taxTypeId: zeroType.id }});
            const exemptType = await prisma.taxType.create({ data: { name: 'VAT_EXEMPT', taxFrameworkId: taxFramework.id } });
            await prisma.taxRate.create({ data: { name: 'UAE Exempt 0%', rate: 0, effectiveFrom: new Date('2020-01-01'), taxTypeId: exemptType.id }});
            const rcType = await prisma.taxType.create({ data: { name: 'VAT_REVERSE_CHARGE', taxFrameworkId: taxFramework.id } });
            await prisma.taxRate.create({ data: { name: 'UAE RC 5%', rate: 5, effectiveFrom: new Date('2020-01-01'), taxTypeId: rcType.id }});
        }
        ...
        if (c.code === 'AU') {
        ...
        } else if (c.code === 'SG' || c.code === 'SA' || c.code === 'JP' || c.code === 'ZA') {
            const label = c.code === 'SG' ? 'SG' : (c.code === 'SA' ? 'SA' : (c.code === 'JP' ? 'JP' : 'ZA'));
            let note = '';
            ...
            else if (c.code === 'ZA') note = 'ZA reverse charge applies to imported services from foreign suppliers. Not a broad domestic mechanism.';
