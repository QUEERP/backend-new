...
async function testDebitNoteTax() {
  ...
  try {
    ...
    console.log("Original Bill Tax Transactions:", billTaxTransactions);
    ...
    TaxResolver.resolveTaxRule = async function(args) {
       console.log("TaxResolver called for Debit Note with transactionType:", args.transactionType);
       const result = await originalResolveTaxRule.apply(this, arguments);
       console.log("TaxResolver returned:", result);
       return result;
    };

    // 2. Create a partial Debit Note (returning 1 quantity)
