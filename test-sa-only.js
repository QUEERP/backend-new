...
const TEST_COUNTRIES = [{ code: 'SA', fw: 'Saudi Arabia VAT' }];
...
async function runTests() {
  try {
    for (const c of TEST_COUNTRIES) {
        ...
        } else if (c.code === 'SG' || c.code === 'SA') {
            const label = c.code === 'SG' ? 'SG' : 'SA';
            const note = c.code === 'SG'
                ? 'SG reverse charge applies to IMPORTED services from overseas non-GST-registered suppliers (not domestic vendors).'
                : 'SA reverse charge applies to imported services from non-SA-registered foreign suppliers (ZATCA VAT law). Not a broad domestic mechanism.';
